...
from .types import ChunkRef, GraphEdge, GraphEdgeType
...
class GraphBuilder:
    ...
    def build(
        ...
        chunk_refs = [ChunkRef(chunk_id=chunk.id, chunk=chunk) for chunk in chunks]

        edges: list[GraphEdge] = []
        if self.include_parents:
            edges.extend(self._parent_edges(chunk_refs))
        edges.extend(self._continuation_edges(chunk_refs))

        symbol_index = self._build_symbol_index(chunk_refs)
        edges.extend(self._reference_edges(chunk_refs, symbol_index))

        return edges

    def _parent_edges(self, chunks: Sequence[ChunkRef]) -> list[GraphEdge]:
        lookup = {ref.chunk_id: ref for ref in chunks}
        edges: list[GraphEdge] = []

        for ref in chunks:
            parent_id = ref.chunk.parent_chunk_id
            if not parent_id:
                continue
    ...
    def _continuation_edges(self, chunks: Sequence[ChunkRef]) -> list[GraphEdge]:
        grouped: dict[int, list[ChunkRef]] = defaultdict(list)

        for ref in chunks:
            if ref.chunk.node is None:
                continue
            grouped[id(ref.chunk.node)].append(ref)
        ...
        for refs in grouped.values():
            ...
            ordered = sorted(
                refs,
                key=lambda r: (r.chunk.start_line, r.chunk.end_line),
            )
            for left, right in zip(ordered, ordered[1:]):
                edges.append(
                    GraphEdge(
                        source_id=left.chunk_id,
                        target_id=right.chunk_id,
    ...
    def _build_symbol_index(
        self, chunks: Sequence[ChunkRef]
    ) -> dict[tuple[str, str], list[tuple[ChunkRef, str | None]]]:
        index: dict[tuple[str, str], list[tuple[ChunkRef, str | None]]] = defaultdict(
            list
        )
        ...
        for ref in chunks:
            ...
            for ident in self._definition_name_nodes(node):
                ...
                if not name:
                    continue
                key = (ref.chunk.language, name)
                owner = self._owner_name(node)
                index[key].append((ref, owner))
    ...
    def _reference_edges(
        ...
        symbol_index: dict[tuple[str, str], list[tuple[ChunkRef, str | None]]],
    ) -> list[GraphEdge]:
        edges: list[GraphEdge] = []
        ...
        for ref in chunks:
            ...
            for candidate in self._reference_candidates(node, context):
                if not candidate.name:
                    continue
                matches = symbol_index.get((ref.chunk.language, candidate.name))
                if not matches:
                    continue
                ...
                for target, owner in matches:
                    if candidate.owner and owner and owner != candidate.owner:
                        continue
                    if target.chunk_id == ref.chunk_id:
                        continue
                    same_file = target.chunk.file_path == ref.chunk.file_path
                    if same_file and not self.include_intra_file_refs:
                        continue
                    ...
                    key = (edge_type, ref.chunk_id, target.chunk_id)
                    if key in seen:
                        continue
                    seen.add(key)
                    edges.append(
                        GraphEdge(
                            source_id=ref.chunk_id,
